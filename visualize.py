...
class Visualizer:
    ...
    #     data = []
    #     ringmaster_id = None
    #     # Determine ringmaster id once from peers
    #     for peer in self.peers.values():
    #         if peer.is_ringmaster:
    #             ringmaster_id = peer.peer_id
    #             break
    ...
    #         longest_chain_length = len(peer.current_longest_chain)
    #         malicious_blocks_in_lc = sum(1 for block in peer.current_longest_chain if block.is_malicious)
    #         # Here, total_blocks_by_malicious is taken from the ringmaster's count of blocks mined.
    #         # It is assumed that the ringmaster maintains a counter named blocks_mined.
    #         total_blocks_by_malicious = self.peers[ringmaster_id].blocks_mined if ringmaster_id in self.peers and hasattr(self.peers[ringmaster_id], 'blocks_mined') else 0
    #         ratio1 = (malicious_blocks_in_lc / max(1, longest_chain_length))
    #         ratio1 = f"{ratio1:.5f}"
    #         ratio2 = (malicious_blocks_in_lc / max(1, total_blocks_by_malicious))
    #         ratio2 = f"{ratio2:.5f}"
    ...
    #     df = pd.DataFrame(data, columns=['Peer ID', 'LC Length', 'Malicious Blocks in LC', 
    #                                     'Total Blocks by Malicious', 
    #                                     'Ratio (Malicious Blocks/LC Length)', 
    #                                     'Ratio (Malicious Blocks/Total Blocks by Malicious Nodes)'])
    ...
    #     # Iterate over all cells and highlight the ringmaster's row.
    #     for (i, j), cell in table.get_celld().items():
    #         if i == 0:
    #             cell.set_facecolor('yellow')
    #             cell.set_text_props(weight='bold', color='black')
